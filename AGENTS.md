...
| Strix Halo RDNA3.5 tuning (ROCm/HIP) | gaetan-puleo via `perf(rocm): apply RDNA3.5 Strix Halo tuning` | PR #21344 closed upstream (not merged); CachyLLama-only | None. Re-evaluate when upstream MMQ/FA tuning for gfx1151 lands. Track: community forks (chrismcmacken, justinappler) carry forward the work. |
| `common::host_available_ram()` | Extracted from `common/kv-ssd-cache.cpp` and the now-removed `kv_page_manager.cpp` (duplicate implementations) | None (CachyLLama-only refactor) | New files `common/host-ram.h` and `common/host-ram.cpp`. Both callers migrated. `kv_page_manager` deleted (superseded by `kv_ssd_cache`). |
